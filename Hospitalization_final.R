...
c.Hosp        <- 63                                         # daily costs of hospitalization 
...
Costs <- function (M_it, Streatment, Int= FALSE,m.length) {
  # M_it: health state occupied by individual i at cycle t (character variable)
...
# Hospitalization
Costs_h <- function(m.length) {
  # 
  # 
  
  c_h <- c.Hosp*m.length                                                              
  
  return(c_h)                                                               # return the costs     	
}
...
#### Function checking decline scheme (for transition probabilities estimates)
decline_scheme = function(MMSE, v.Scheme, v.Switch, v.Streatment){   
  # MMSE: cognitive score in previous cycle
  # vStreatment: has the treatment been initiated? 
  # Int: is the individual in the intervention branch? (default is FALSE) 
...
Diff_mild <- function(MMSE) { 
  d_mild <- MMSE - 21   		                                 # return the transition probabilities 
  return(d_mild)
}  

Diff_severe <- function(MMSE) { 
  d_severe <-  MMSE - 11  		                                # return the transition probabilities
  return(d_severe)
}

#### Function updating hospitalization status during the cycle 
Hosp <- function(M_it, t) {
  
  m.p.h          <- matrix(0, nrow = n.h, ncol = n.i)     # create matrix of state transition probabilities
  rownames(m.p.h) <-  v.hosp                              # give the state names to the rows
  
  m.p.h[,M_it == "Mild"]     <- rbind(0.02393964, 1-0.02393964)
  m.p.h[,M_it == "Moderate"] <- rbind(0.03710945, 0.9628906)
  m.p.h[,M_it == "Severe"]   <- rbind(0.1116529, 0.8883471)
  m.p.h[,M_it == "Death"]      <- rbind(0, 1)
  
  return(t(m.p.h))
}

#### Function drawing hospitalization length 
Length <- function(m.hosp){
  
  h    <- sum(m.hosp == "Hosp")              
  length = ifelse(m.hosp == "Hosp", rlnorm(n = h, mean = log(21.9), sd = log(3.74)), 0)
  length = ifelse(length > 365, 365, length)
...
MicroSim <- function(n.i, df.X , Int = FALSE, seed = 1) {
  ...
  m.c.hosp  <- m.length <- m.hosp <- m.decline_severe <- m.decline_mild <- m.scheme <- m.F <- m.c.informal <- m.c.outpatient <- m.c.screening <- m.c.medication <- m.M <- m.C <- m.E <- m.Streatment <- m.MMSE <- matrix(nrow = n.i, ncol = n.t + 1, 
  ...
  m.C[, 1]          <- Costs(m.M[, 1], m.Streatment[,1], Int, m.length[,1])                       # calculate costs per individual during cycle 0
  ...
  m.c.hosp[, 1] <- Costs_h(m.length[,1])
  m.F[, 1]<- Fraction(m.M[, 1], m.Streatment[,1])                                                 # calculate outpatient costs per individual during cycle 0
  m.scheme[,1] <- decline_scheme(df.X$MMSE, v.Scheme, v.Switch, m.Streatment[,1])
  m.decline_mild[,1] <- Diff_mild(v.MMSE)
  m.decline_severe[,1] <- Diff_severe(v.MMSE)
  ...
  for (t in 1:n.t) { 
    ...
    v.ph            <- Hosp(m.M[, t], t)
    m.hosp[,t + 1]    <- samplev(v.ph, 1)
    m.length[, t + 1] <- Length(m.hosp[,t + 1])
    df.X$MMSE       <- upMMSE(df.X$MMSE, v.Scheme, v.Switch, m.Streatment[,t])                    # calculate mmse score during the cycle t + 1
    m.scheme[,t + 1] <- decline_scheme(df.X$MMSE, v.Scheme, v.Switch, m.Streatment[,t])
    m.decline_mild[, t +1] <- Diff_mild(df.X$MMSE)
    m.decline_severe[,t + 1] <- Diff_severe(df.X$MMSE)
    m.MMSE[, t + 1] <-  df.X$MMSE                                                                 # store MMSE score to the matrix
    m.C[, t + 1]    <- Costs(m.M[, t + 1], m.Streatment[,t], Int, m.length[,t +1])                # calculate costs per individual during cycle t + 1
    ...
    m.c.hosp[, t + 1] <- Costs_h(m.length[,t + 1])
    ...
    m.E[, t + 1]    <- ifelse(m.M[, t + 1] != "Death" & m.hosp[,t + 1] != "Hosp", 0.408+0.01*df.X$MMSE,
                              ifelse(m.M[, t + 1] != "Death" & m.hosp[,t + 1] == "Hosp", 0.408+0.01*df.X$MMSE - 0.159,
  ...
  tc_hat <- mean(tc)
  te_hat <- mean(te)
  results <<- list(hosp = hosp, m.c.hosp = m.c.hosp, m.length = m.length, m.hosp = m.hosp, m.decline_mild = m.decline_mild, m.decline_severe = m.decline_severe, m.scheme = m.scheme, m.Streatment = m.Streatment, m.MMSE = m.MMSE, m.M = m.M, m.C = m.C, m.E = m.E, tc = tc , te = te, tc_hat = tc_hat, te_hat = te_hat, m.c.informal = m.c.informal, m.c.outpatient = m.c.outpatient, m.c.screening = m.c.screening, m.c.medication = m.c.medication, ti = ti, to = to, ts = ts, tm = tm, m.F = m.F) # store the results from the simulation in a list  
...
title(main = c(" "," ",""))
legend("topright", v.n, col=1:4,                                  # add a legend to current plot
...
checkHosp_i <- outcomes_int[["m.c.hosp"]]
...
checkD_i <- outcomes_int[["m.scheme"]]
checkMild_i <- outcomes_int[["m.decline_mild"]]
checkSevere_i <- outcomes_int[["m.decline_severe"]]
checkH_i <- outcomes_int[["m.hosp"]]
checkL_i <- outcomes_int[["m.length"]]
hospC_i <- outcomes_int[["m.c.hosp"]]
...
checkHosp_cau <- outcomes_no_int[["m.c.hosp"]]
...
checkD_cau <- outcomes_no_int[["m.scheme"]]
checkMild_cau <- outcomes_no_int[["m.decline_mild"]]
checkSevere_cau <- outcomes_no_int[["m.decline_severe"]]
checkH_cau <- outcomes_no_int[["m.hosp"]]
checkL_cau <- outcomes_no_int[["m.length"]]
hospC_cau <- outcomes_no_int[["m.c.hosp"]]
...
# Cost categories
# Table 2.18
c_informal_cau <- sum(ti_cau)/100000
c_informal_i <- sum(ti_i)/100000
c_screening_cau <- sum(ts_cau)/100000
c_screening_i <- sum(ts_i)/100000
c_outpatient_cau <- sum(to_cau)/100000
c_outpatient_i <- sum(to_i)/100000
c_medication_cau <- sum(tm_cau)/100000
c_medication_i <- sum(tm_i)/100000
c_hospitalization_cau <- sum(hospC_cau)/100000
c_hospitalization_i <- sum(hospC_i)/100000

total_cau = c_informal_cau + c_screening_cau + c_outpatient_cau + c_medication_cau + c_hospitalization_cau
total_i = c_informal_i + c_screening_i + c_outpatient_i + c_medication_i + c_hospitalization_i

cau <- c_informal_cau + c_screening_cau + c_outpatient_cau + c_medication_cau
i <- c_informal_i + c_screening_i + c_outpatient_i + c_medication_i
cau_hc <- c_screening_cau + c_outpatient_cau + c_medication_cau
i_hc <- c_screening_i + c_outpatient_i + c_medication_i
cau - i
hc <- cau_hc - i_hc
informal <- c_informal_cau - c_informal_i
outpatient <- c_outpatient_cau - c_outpatient_i
screeing <- c_screening_cau - c_screening_i
medication <- c_medication_cau - c_medication_i
...
# probability of hospitalization according to formula 2.7 as inserted in Hosp function
0.038/(370398/739168)*0.06
0.396/(143947/224823)*0.06
0.566/(224823/739168)*0.06

0.038/(70398/739168)*0.06
0.396/(143947/224823)*0.06
0.566/(224823/739168)*0.06


# share of hospitalized people
temp_i <- as.data.frame(checkH_i)
temp_cau<- as.data.frame(checkH_cau)
alive_i <- 323976   + 196541    + 116545 
alive_cau <- 270560   + 143472  + 222616 
sum(colSums(temp_i[-1] == "Hosp"))/alive_i
sum(colSums(temp_cau[-1] == "Hosp"))/alive_cau
