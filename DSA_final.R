...
#temp <- runif(n.i, 21, 22)                                 # help for the DSA to have the same base case values
tmmse   <-  runif(n.i,11, 21)                               # base case runif(n.i,11, 21), DSA 21, 11 ;MMSE of diagnosis and treatment initiation (SA tmmse19, SAimmse 20)
#temp <- runif(n.i, 21, 22)                                 # help for the DSA to have the same base case values
immse   <- runif(n.i,21, 28)                                # base case runif(n.i,21, 28), DSA 21, 28 
distr   <- read.csv("sampling.csv") %>% 
  janitor::clean_names() %>% 
  mutate(age = onset)

# load age at disease onset distribution
p.Treatment <- 0.5 #base case 0.5, DSA 0.75, 1              # indicates the probability of receiving treatment without intervention
p.Treatment_i <- 1
...
c.Moderate.i  <- 1872*12*((1+0.03)^3) #DSA 13885              # annual costs of indirect care for moderate cognitive deficit
c.Severe.i    <- 2155*12*((1+0.03)^3) #DSA 13885              # annual costs of indirect care for severe cognitive deficit
...
c.Screening   <- (97*((1+0.03)^3) + (26 + 18)/((1+0.03)^2))*31# costs of screening (for treated patients in the intervention branch) DSA 25tt and 10000
...
anb = benefit_i - benefit_cau
benefit_i - benefit_cau
