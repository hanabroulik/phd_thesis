...
p.Death <- read.csv("ut.csv") %>%                         # load Czech mortality tables and adjust by HR
  mutate(Male = (1 - exp(-(- log(1 - qxM))*hr*1/c)), Female = (1 - exp(-(- log(1 - qxZ))*hr*1/c))) %>% 
  select(age, Male, Female) %>% 
  gather(key = "sex" , value = "p.Mortality", 2:3) 


#### Cost inputs (unit costs discounted to the value of 2017)
c.Mild.i      <- 951*12*((1+0.03)^3)/c                        # annual costs of indirect care for mild cognitive deficit
c.Moderate.i  <- 1872*12*((1+0.03)^3)/c                       # annual costs of indirect care for moderate cognitive deficit
c.Severe.i    <- 2155*12*((1+0.03)^3)/c                       # annual costs of indirect care for severe cognitive deficit
c.Outpatient  <- 97*((1+0.03)^3)/c                            # annual costs of outpatient care (for treated patients)
c.Mild.d      <- 93*2*((1+0.03)^5)/c                          # annual costs of drugs for mild cognitive deficit (for treated patients)
c.Moderate.d  <- 93*2*((1+0.03)^5)/c                          # annual costs of drugs for moderate cognitive deficit (for treated patients)
c.Severe.d    <- 289*2*((1+0.03)^5)/c                         # annual costs of drugs for severe cognitive deficit (for treated patients)
...
df.X  <- merge(df.X, distr, by = "age") %>% 
  mutate(sex = ifelse(rg<freq_gen, "Male", "Female"), age_real = age) %>% 
  select(age, sex, MMSE, age_real)                                    # merging probablity of being man to the dataframe, creating gender
...
upMMSE = function(MMSE, v.Scheme, v.Switch, v.Streatment, c){   
  ...
  d.MMSE = MMSE
  d.MMSE[decline == "treated_mean"] = rtruncnorm(tm, a = 0, b = Inf, mean = 1.5, sd = 1.5)/c
  d.MMSE[decline == "untreated_mean"] = rtruncnorm(utm, a = 0, b = Inf, mean = 3.5, sd = 1.5)/c
  d.MMSE[decline == "treated_lopez_slow"] = runif(tls, min = -1, max = 2)/c
  d.MMSE[decline == "untreated_lopez_slow"] = runif(utls, min = -1, max = 2)/c
  d.MMSE[decline == "treated_lopez_fast"] = runif(tlf, min = 3, max = 5)/c
  d.MMSE[decline == "untreated_lopez_fast"] = runif(utlf, min = 3, max = 6.8)/c 
...
MicroSim <- function(n.i, df.X , Int = FALSE, seed = 1, c) {
  # Arguments:  
  ...
  m.age <- m.F <- m.c.informal <- m.c.outpatient <- m.c.screening <- m.c.medication <- m.M <- m.C <- m.E <- m.Streatment <- m.MMSE <- matrix(nrow = n.i, ncol = n.t + 1, 
  ...
  for (t in 1:n.t) { 
    ...
    df.X$MMSE       <- upMMSE(df.X$MMSE, v.Scheme, v.Switch, m.Streatment[,t], c)                 # calculate mmse score during the cycle t + 1
    ...
    m.E[, t + 1]    <- ifelse(m.M[, t + 1] != "Death", (0.408+0.01*df.X$MMSE)/c,0)                # calculate QALYs per individual during cycle t + 1
    ...
    # update age
    df.X$age_real[m.M[,t + 1] != "Death"] <- df.X$age_real[m.M[, t + 1] != "Death"] + 1/c
    df.X$age <- round(df.X$age_real)
    m.age[, t + 1] <- df.X$age
  ...
  tc_hat <- mean(tc)
  te_hat <- mean(te)
  results <<- list(m.age = m.age, m.Streatment = m.Streatment, m.MMSE = m.MMSE, m.M = m.M, m.C = m.C, m.E = m.E, tc = tc , te = te, tc_hat = tc_hat, te_hat = te_hat, m.c.informal = m.c.informal, m.c.outpatient = m.c.outpatient, m.c.screening = m.c.screening, m.c.medication = m.c.medication, ti = ti, to = to, ts = ts, tm = tm, m.F = m.F) # store the results from the simulation in a list  
...
outcomes_no_int  <- MicroSim(n.i, df.X, Int = FALSE, seed = 1, c)
outcomes_int     <- MicroSim(n.i, df.X, Int = TRUE, seed = 1, c)
...
# accuracy
diagnosis <- as.data.frame(checkF_i) %>%
  select(- "cycle 0") %>% 
  mutate(last = 0)
real_immse <- as.matrix(diagnosis) * checkMMSE_i
temp1 <- rowSums(real_immse[,1:41] ) # pro každého, s jakým MMSE byl diagnostikovaný
temp2 <- cbind(temp1, immse) %>% as.data.frame() %>% 
  mutate(diff = immse - temp1, treat = rowSums(checkF_i[,1:41] )) %>% 
  filter(treat == 1) %>% 
  mutate(tmmse = immse) %>% 
  select(-immse)
# temp3 <- filter(temp2, immse > temp1) 


diagnosis_cau <- as.data.frame(checkF_cau) %>%
  select(- "cycle 0") %>% 
  mutate(last = 0)
real_tmmse <- as.matrix(diagnosis_cau) * checkMMSE_cau
temp_cau1 <- rowSums(real_tmmse[,1:41] )
temp_cau2 <- cbind(temp_cau1, tmmse) %>% as.data.frame() %>% 
  mutate(diff = tmmse - temp_cau1, treat = rowSums(checkF_cau[,1:41] )) %>% 
  filter(treat == 1) %>% 
  mutate(temp1 = temp_cau1) %>% 
  select(-temp_cau1)


temp4 <- rbind(temp_cau2,temp2)
summary(temp4$diff)
summary(temp2$diff)
temp5 <- filter(temp2, temp1 < 21)
temp6 <- filter(temp_cau2, temp1 < 11)
